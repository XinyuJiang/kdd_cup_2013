...
class Trainer(object):
    ...
    def make_feature_file(self, train_AuthorIdPaperIds, test_AuthorIdPaperIds, dict_coauthor, dict_paperIdAuthorId_to_name_aff, dict_author_keywords, dict_author_conference_journal, dict_author_cj_keywords, Conference, Journal, PaperAuthor, Author, Paper):
        ...
        Make_feature_file(train_AuthorIdPaperIds, dict_coauthor, dict_paperIdAuthorId_to_name_aff, dict_author_keywords, dict_author_conference_journal, dict_author_cj_keywords, Conference, Journal, PaperAuthor, Author, Paper, self.feature_function_list, self.train_feature_path)
        print("make test feature file ...")
        Make_feature_file(test_AuthorIdPaperIds, dict_coauthor, dict_paperIdAuthorId_to_name_aff, dict_author_keywords, dict_author_conference_journal, dict_author_cj_keywords, Conference, Journal, PaperAuthor, Author, Paper, self.feature_function_list, self.test_feature_path)
...
if __name__ == "__main__":
    ...
    dict_author_conference_journal = json.load(open(config.AUTHOR_CONFERENCE_JOURNAL_FILE), encoding="utf-8")
    dict_author_cj_keywords = json.load(open(config.AUTHOR_CJ_KEYWORDS), encoding="utf-8")
    # 使用pandas加载csv数据
    ...
    trainer.make_feature_file(train_AuthorIdPaperIds, test_AuthorIdPaperIds, dict_coauthor, dict_paperIdAuthorId_to_name_aff, dict_author_keywords, dict_author_conference_journal, dict_author_cj_keywords, Conference, Journal, PaperAuthor, Author, Paper)
    # 根据训练特征文件，训练模型
    trainer.train_mode()
    # 使用训练好的模型，对测试集进行预测
    trainer.test_model()
